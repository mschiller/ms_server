...
bash 'Unpack Jetty' do
  code   "tar -xzvf #{node.jetty.download} -C #{node.jetty.directory}"
...
%w(jetty.xml webdefault.xml jetty-deploy.xml jetty-logging.xml ).each do |f| #jetty-annotations.xml jetty-testrealm.xml jetty-contexts.xml jetty-webapps.xml
