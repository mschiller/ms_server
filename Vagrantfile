...
Vagrant::Config.run do |config|
  ...
  config.vm.define :application_server do |web_config|
    ...
    node['mysql']['bind_address'] = '127.0.0.1'

    # Every Vagrant virtual environment requires a box to build off of.
    web_config.vm.box = 'ubuntu-1004-server-i386'
    web_config.vm.guest = :ubuntu
    ...
    web_config.vm.provision :chef_solo do |chef|
      ...
      node.delete("recipes").each do |recipe|
    ...
    # Forward a port from the guest to the host, which allows for outside
    # computers to access the VM, whereas host only networking does not.
    web_config.vm.forward_port(80, 8880)
    web_config.vm.forward_port(443, 8443)
    web_config.vm.forward_port(21, 8821)
    web_config.vm.forward_port(22, 8822, :auto => true)
    #web_config.vm.forward_port(3306, 3333)
    #web_config.vm.forward_port(8981, 9984)
