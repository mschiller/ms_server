...
Vagrant::Config.run do |config|
  ...
  node = Automation::ChefHelper.node_settings('config.vagrant')
  ...
  config.vm.provision :chef_solo do |chef|
    chef.cookbooks_path = [File.join('chef', 'site-cookbooks')]
    chef.add_recipe "framework"
  ...
  config.vm.forward_port("dev", 3000, 4000)

  # Share an additional folder to the guest VM. The first argument is
  # an identifier, the second is the path on the guest to mount the
  # folder, and the third is the path on the host to the actual folder.
  # set current project folder for work with passenger
  config.vm.share_folder("current_project", node['application']['deploy_to'], ".")
